...
function InputSection(props: InputSectionProps) {
  ...
  return (
    <div class="max-w-4xl mx-auto">
      <div class="download-box rounded-lg">
        <div class="bg-white rounded-lg backdrop-blur-md p-1">
          <form class="flex flex-col md:flex-row items-stretch md:items-center gap-2"
            onSubmit={props.onSubmit}
          >
            <div class="relative flex-grow rounded-lg bg-grey-200">
              <input type="text"
                value={props.url}
                ...
                class="w-full h-14 border-gray-200 text-black rounded-lg px-5 pr-20 focus:outline-none focus:border-transparent transition-all duration-300"
              />
              <button type="button" 
                onClick={props.onPaste} 
                class="absolute right-3 top-1/2 transform -translate-y-1/2 bg-white shadow text-blue-600 px-2 py-2 transition-all duration-300 flex items-center">
            ...
            <button type="submit" 
              ...
              class="h-14 px-8 bg-blue-600 disabled:from-gray-500 disabled:to-gray-400 text-white font-medium rounded-lg flex items-center justify-center gap-2 disabled:cursor-not-allowed">
              {props.loading ? (
                <>
